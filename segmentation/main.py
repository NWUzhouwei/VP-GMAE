...
def parse_args():
    ...
    parser.add_argument('--batch_size', type=int, default=8, # 16, 32
    ...
    parser.add_argument('--resume', action='store_true',
                        default=False, help='use normals')
    parser.add_argument(
        '--ckpts', type=str, default='runs\\pretrain-VSSL\\cfgs\\exp\\ckpt-last.pth', help='ckpts')
    parser.add_argument(
        '--root', type=str, default='D:\\datasets\\shapenetcore_partanno_segmentation_benchmark_v0_normal', help='data root')
...
def main(args):
    ...
    exp_dir.mkdir(exist_ok=True)
    checkpoints_dir = exp_dir.joinpath('checkpoints/') # model save path
    ...
    if args.resume :
        last_checkpoint_path = os.path.join(checkpoints_dir, "last_model.pth")
        if os.path.isfile(last_checkpoint_path):
            checkpoint = torch.load(last_checkpoint_path)
            classifier.load_state_dict(checkpoint['model_state_dict'])
            start_epoch = checkpoint['epoch'] + 1
            best_acc = checkpoint['best_acc']
            best_class_avg_iou = checkpoint['best_class_avg_iou']
            best_inctance_avg_iou = checkpoint['best_inctance_avg_iou']
            log_string("Load last checkpoint from" + str(last_checkpoint_path))
        else:
            log_string("Can not resume!")
    else:    
    ...
    for epoch in range(start_epoch, args.epoch):
        ...
        num_iter = 0
        '''learning one epoch'''
        for i, (points, label, target) in tqdm(enumerate(trainDataLoader), total=len(trainDataLoader), desc="Train", ncols=0):
            num_iter += 1
        ...
        with torch.no_grad():
            ...
            for batch_id, (points, label, target) in tqdm(enumerate(testDataLoader), total=len(testDataLoader), desc="Test", ncols=0):
        ...
        if test_metrics['inctance_avg_iou'] > best_inctance_avg_iou:
            ...
            log_string('Best model saving at %s' % savepath)
        ...
        state = {
            ...
            'best_inctance_avg_iou': best_inctance_avg_iou,
        }
        torch.save(state, savepath)
        log_string('Last model saving at %s' % savepath)
