...
#include "gsvoxelization/gsvox.hpp"
#include "maxvoxelization/maxvox.hpp"
...
PYBIND11_MODULE(_pvcnn_backend, m) {
  ...
  m.def("gs_voxelize_forward", &gs_voxelize_forward,
        "GSVoxelization forward with average pooling (CUDA)");
  m.def("gs_voxelize_backward", &gs_voxelize_backward,
        "GSVoxelization backward (CUDA)");
  m.def("max_voxelize_forward", &max_voxelize_forward,
        "MaxVoxelization forward with average pooling (CUDA)");
  m.def("max_voxelize_backward", &max_voxelize_backward,
        "MaxVoxelization backward (CUDA)");
}
