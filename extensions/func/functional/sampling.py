...
def furthest_point_sample(coords, features , num_samples): # changed
    """
    ...
    indices = _backend.furthest_point_sampling(coords, num_samples) # b m
    if features == None :
        return gather(coords, indices)
    else:
        return gather(coords, indices), gather(features, indices) # (_, add)
