...
def run_net(args, config, train_writer=None, val_writer=None):
    ...
    for epoch in range(start_epoch, config.max_epoch + 1):
        ...
        idx = 0
        for taxonomy_ids, model_ids, data in tqdm(train_dataloader, desc="Train", ncols=0): # enumerate(train_dataloader):
            num_iter += 1
        ...
        print_log('[Best acc]  %.4f' % (best_metrics.acc), logger=logger)
...
def validate(base_model, test_dataloader, epoch, val_writer, args, config, logger=None):
    ...
    npoints = config.npoints
    with torch.no_grad():
        for taxonomy_ids, model_ids, data in tqdm(test_dataloader, desc="Test", ncols=0): # enumerate(test_dataloader):
            points = data[0].cuda()
            label = data[1].cuda()
...
def validate_vote(base_model, test_dataloader, epoch, val_writer, args, config, logger=None, times=10):
    ...
    npoints = config.npoints
    with torch.no_grad():
        for taxonomy_ids, model_ids, data in tqdm(test_dataloader, desc="Test_vote", ncols=0): # enumerate(test_dataloader):
